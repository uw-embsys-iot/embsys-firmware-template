...
static bool encode_ota_update_request(uint8_t *buffer, size_t buffer_size, size_t *message_length)
{
	...
	message.state = OTAState_OTA_STATE_IN_PROGRESS;
...
int http_ota_proto_payload_get(uint8_t* buffer, size_t buf_size) {
	size_t message_length;
...
void http_ota_proto_response_cb(struct http_response *rsp,
			enum http_final_call final_data,
			void *user_data)
{
	if (final_data == HTTP_DATA_MORE) {
...
static void backend_ota_http_request(void) {
	...
	// The OTA payload is fairly small.
	uint8_t payload_buf[20];
...
static void http_ota_request() {
	...
	if (connect_socket(AF_INET, kOtaReleaseServer, OTA_HTTP_PORT,  &sock, (struct sockaddr *)&addr4, sizeof(addr4)) < 0) {
		LOG_ERR("Connect failed");
